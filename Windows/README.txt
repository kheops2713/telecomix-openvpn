...
i. Edit stunnel.conf and put your endpoint's stunnel IP and 
   port in the 'connect' parameter. Then place stunnel.conf file in: 
    Program Files\stunnel

ii. Edit openvpn-stunnel.conf and put your endpoint's IP in
    the 'route' line. Then place openvpn-stunnel.conf in:
...
Note:
Paths may differ from those explained above if you didn't install
any of the required softwares in the default path, or if you had
an x64 system. In the last case, the paths will be:
    Program Files (x86)\...
