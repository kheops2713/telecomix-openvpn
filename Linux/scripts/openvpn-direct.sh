...
print_center "OpenVPN client starting"
...
# 2. Start OpenVPN
echo -e 'a\nb' >/tmp/auth-$id
options="--daemon --client --dev tun --proto tcp --remote $host $port --persist-tun --script-security 3 --auth-user-pass /tmp/auth-$id --writepid $pidfile --log $logfile --comp-lzo --ping 5 --verb 3 --ca $pwd/wnh-ca.crt --pull"
if [ "$full" != y ]; then
