...
# Client-side script for OpenVPN over Stunnel for Tcx Darknet
...
STUNNEL=$(which stunnel4 2>/dev/null || which stunnel 2>/dev/null)
[ -z "$STUNNEL" ] && print_error "Could not find stunnel/stunnel4 executable" && exit
...
stunnellog=/tmp/stunnel.log-$id
stunnelpidfile=/tmp/stunnel.pid-$id
stunnelconffile=/tmp/stunnel.conf-$id
stunnelconf="
client = yes
foreground = no
verify = 2
...
print_center "OpenVPN over Stunnel client starting"
...
echo "$stunnelconf" >$stunnelconffile
$STUNNEL $stunnelconffile
sleep 5
kill -0 $(cat $stunnelpidfile) 2>/dev/null
if [ $? = 0 ]; then
    echo $(print_right ok)
else
...
stunnelpid=$(cat $stunnelpidfile)

# 3. Start OpenVPN
# IMPORTANT Note: we set cipher to "none" to avoid double encryption (and thus overhead), as we have Stunnel's SSL wrap around OpenVPN
echo -e 'a\nb' >/tmp/auth-$id
options="--daemon --client --dev tun --proto tcp --remote 127.0.0.1 5043 --route $host 255.255.255.255 net_gateway --persist-tun --script-security 3 --auth-user-pass /tmp/auth-$id --writepid $pidfile --log $logfile --comp-lzo --ping 5 --verb 3 --ca $pwd/wnh-ca.crt --pull"
if [ "$full" != y ]; then
